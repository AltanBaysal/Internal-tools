...
**Tarih:** 2026-09-11 · **Koşu dalı:** `feat/queen-editor-v5` · **Durum:** 5/9
...
| 214 | **Slime girl videosu eklenecek.** *(Kullanıcı, 6 Eylül.)* Bir video türü — *slime girl*. Bir model değil, üretilecek bir içerik biçimi. **Kararlaşmadı:** kendi LoRA'sıyla mı geliyor, kendi üretim tarifiyle mi, yoksa yalnız prompt tarafında mı kalıyor. **Kullanıcıdan gereken** *(spec'in başında istenir)*: bu karar; yol LoRA ise dosyanın indirilip verilmesi; ve çıkan videoya bakması. Grafik tarafında yer hazır: video grafiğinde **iki Power Lora Loader** var *(high ve low noise geçişleri)* ve her birinde `lora_1` hız LoRA'sı, `lora_2` SmoothMix Animations takılı — üçüncüsü `lora_3`'e girer. Şartı, LoRA'nın **WAN 2.2 I2V** tabanlı olması ve **High + Low çifti** gelmesi; T2V tabanlı bir dosya bu grafiğe uymuyor *(`wan22-smooth-t2v` denemesinde bir kez düşülen tuzak)*. | Karar yazıya geçmiş, ve o yolla üretilmiş bir slime girl videosunu kullanıcı görmüş. |
