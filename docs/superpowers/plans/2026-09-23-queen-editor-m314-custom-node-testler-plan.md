# Madde 314 — Custom node hücresi kısalıyor, test turunun planı

> **Koşum:** bu oturumda, satır satır. Alt ajan yok *(CLAUDE.md, Gotchas)*.

**Hedef:** Impact-Pack'in `sam2`'siz kurulduğunu, konsolun bunu söylediğini, başka listelere
dokunulmadığını, node kurulumunun bugünkü davranışını ve defterin onu `install_node` ile çağırdığını
anlatan sekiz test — kırmızı.

**Yaklaşım:** `colab/nodes.py` henüz yok; yedi test onu `colab/downloads.py`'nin testleri gibi, ağı
sahte ve klasörleri gerçek koşuyor. Git ve pip `nodes.run`'ın yerine geçen tek bir sahtede. Defterin
custom node hücresi okunuyor.

**Araçlar:** pytest (`monkeypatch`, `tmp_path`, `capsys`).

**Spec:** [m314 test turu](../specs/2026-09-23-queen-editor-m314-custom-node-testler-design.md)
...
- `colab.nodes.install_node(name, url, folder)`; `run`'ı `colab.console`'dan alıyor, testler onu
  `nodes.run` olarak değiştiriyor.
- Klon komutu bir liste: `git clone`, `--depth 1`, `--recurse-submodules`, adres, sonda hedef klasör.
- pip'e liste dosyası `-r` ile veriliyor.
- Defterde `for name, url in CUSTOM_NODES:` döngüsü `install_node(name, url, …)` çağırıyor; ad
  `from colab.nodes import install_node` ile geliyor.

---

## Görev 1: `test_colab_nodes.py`

**Dosya:** Oluştur: `queen-editor/backend/tests/test_colab_nodes.py`

- [ ] **Adım 1: Dosya, yedi testle.**
...
- [ ] **Adım 2: `test_the_intro_agrees_with_the_custom_node_list`'in hemen altına.**
...
- [ ] **Adım 1: Dört satırı koş** — `queen-editor` pytest'te yalnız bu sekiz kırmızı: 1–7 kurulumda
  hata (`No module named 'colab.nodes'`), 8 başarısız. Öteki üç satır yeşil.
- [ ] **Adım 2: Kırmızı commit** — iki test dosyası, spec ve bu plan: `test(m314): …(red)`.
