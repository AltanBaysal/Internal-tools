...
_PINNED = {"provider": {"order": ["deepinfra"], "allow_fallbacks": False}}


def test_the_extra_body_fields_reach_the_request():
    """Madde 149: what a model's row needs in the body, sent with every request it makes.

    A body field rather than a header, because that is where OpenRouter reads its provider pin --
    and the pin is what decides whose terms the request runs under.
    """
    seen = {}
...
def test_the_extra_reaches_a_streamed_request_too():
    # The road that matters: every answer the user waits on is streamed, and a pin that held only
    # on `complete` would be a pin that never ran.
    seen = {}
...
def test_the_clients_own_fields_win_over_the_extra():
    """The extra adds; it never takes over.

    A row is data -- it comes from a table a person edits -- and which model a request goes to is
    not a thing data may decide. Written as an ordering rather than a filter: the client's own
    fields go in last, so there is no list of forbidden names to keep up to date.
    """
    seen = {}
    ...
    client = XaiClient(
        ...
        "the-real-model",
        "https://openrouter.ai/api/v1",
        extra={"model": "a-model-nobody-asked-for", "messages": [{"role": "user", "content": "x"}]},
        opener=opener,
    )
    client.complete(MESSAGES)
    assert seen["body"]["model"] == "the-real-model"
    assert seen["body"]["messages"] == MESSAGES


def test_without_an_extra_the_body_is_what_it_has_always_been():
    # The three direct rows carry none, so this is the shape Grok and DeepSeek keep going out in.
    seen = {}
