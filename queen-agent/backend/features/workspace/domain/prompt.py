...
START_A_SCENARIO = (
    ...
    "prompts, in one flow. Six steps, in order; you walk the user through them by "
    "asking.\n"
    "\n"
    ...
    "\n"
    "1. The context. Ask what is being made and what it is for, before anything is written. "
    "Everything after this is built on the answer.\n"
    "\n"
    "2. The plan. A chat's first turn opens with write_plan; later turns "
    "carry on from what the chat already knows. The plan opens with one line of context, so a "
    "fresh chat inherits the work. A "
    "plan already there when the chat opened is that memory: read it and carry on from the step "
    "it left open; with several, ask which. start_scenario opens the file here, once, so every "
    "step after this writes into a file that exists. This step waits for no approval; the next "
    "question follows at once.\n"
    "\n"
    "3. The characters. add_character puts each of them "
    ...
    "\n"
    "4. The places. add_location, the same loop.\n"
    "\n"
    "5. The scenes. Ask how many scenes and which moments matter, then write them with add_scene: "
    ...
    "\n"
    "6. The prompts. write_missing_actions fills every waiting frame in one call, then "
