...
GENERATE_PROMPTS_PLUS = (
    ...
    "file right, call the builder -- and read_prompt_structure_schema once, before the first "
...
START_A_SCENARIO = (
    ...
    "marked done.\n"
    "\n"
    "1. The plan. A chat's first turn opens with list_files, then write_plan; later turns "
    "carry on from what the chat already knows. The plan opens with one line of context -- "
    "what is being made, and for what -- so a fresh chat that reads it inherits the work. A "
    "plan already there is that memory: read it and carry on from the step it left open; with "
    "several, ask which. This step alone waits for no approval; the first question follows at "
    "once.\n"
    "\n"
