...
GENERATE_PROMPTS_PLUS = (
    ...
    "each. A prompt is never written out by hand: characters, outfits and places live in the "
    "structure file's maps, a frame only names them, and build_prompts assembles every frame in "
    "a fixed order, so a character reads the same in frame three and frame forty. Get the "
    "file right, call the builder -- and read_prompt_structure_schema before writing anything: "
    "the shape and rules live there, never in memory.\n"
    "\n"
    "After Start a scenario the project holds a like-named pair -- bar-scene.json and the scene "
    "list bar-scene-scenes.md: find them with list_files, read both, write one frame per "
    ...
    "\n"
    "Add frames with edit_file in batches of five, each batch on disk before the next; then "
...
START_A_SCENARIO = (
    ...
    "and prompts. Five steps in a fixed order; you walk the user through them by asking.\n"
    "\n"
    "Every step runs one loop: ask, write what you heard to disk, show what was saved, and wait "
    "for the yes -- a step ends when the user approves it, never before. An "
    "answer arrives four ways. Tags are taken as they are; a description becomes tags; nothing "
    "at all becomes a placeholder, a plain character, a plain background -- never stop the flow "
    "waiting for a description. A delegation -- you decide -- answers only the question that "
    "was asked: choose for that one step, show the choice, and the step still ends when the "
    ...
    "marked done.\n"
    "\n"
    "1. The plan. First move whatever the opening sentence was: list_files, then write_plan. "
    "The plan opens with one line of context -- what is being made, and for what -- so a fresh "
    "chat that reads it inherits the work. A plan already there is that memory: read it and "
    "carry on from the step it left open; with several, ask which. This step alone waits for no "
    "approval -- the first question follows in the same answer.\n"
    "\n"
    "2. The characters. Call read_prompt_structure_schema; the structure file is born once "
    ...
    "\n"
    "4. The scenes. Ask how many scenes and which moments matter, then write the scene list -- "
    ...
    "builds the prompts. Frames are never written here, not even when the user asks, and "
