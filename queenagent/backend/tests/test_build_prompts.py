...
def test_a_shot_is_built_in_the_fixed_order():
    assert build_prompts(_structure()) == [
        f"{QUALITY}, {AYLIN}, {BEDROOM}, an action, a camera"
    ]


def test_the_character_text_is_the_same_in_every_shot():
    built = build_prompts(_structure(shots=[_shot(action="one"), _shot(action="two")]))
    # The whole point of the structure: no copy, so no drift.
    assert AYLIN in built[0] and AYLIN in built[1]


def test_one_edit_in_the_map_turns_every_shot():
    structure = _structure(shots=[_shot(action="one"), _shot(action="two")])
...
def test_two_characters_keep_the_shots_own_order():
    built = build_prompts(_structure(shots=[_shot(characters=["deniz", "aylin"])]))
    assert built[0].index(DENIZ) < built[0].index(AYLIN)


def test_a_shot_without_a_character_or_a_place_still_builds():
    built = build_prompts(_structure(shots=[_shot(characters=[], location="")]))
...
def test_loose_commas_and_spaces_are_tidied_away():
    structure = _structure(
        quality=" score_9_up , ",
        shots=[_shot(action="", camera=" ,, medium shot,")],
    )
...
def test_a_repeated_solo_tag_is_left_exactly_as_written():
    structure = _structure(shots=[_shot(characters=["aylin", "deniz"])])
...
def test_an_unknown_character_names_the_shot_the_name_and_what_is_known():
    with pytest.raises(BadStructure) as refused:
        build_prompts(_structure(shots=[_shot(characters=["aylinn"])]))
    said = str(refused.value)
    assert "shot 1" in said
...
def test_an_unknown_place_is_reported_the_same_way():
    with pytest.raises(BadStructure) as refused:
        build_prompts(_structure(shots=[_shot(location="rooftop")]))
    said = str(refused.value)
    assert "shot 1" in said and "rooftop" in said and "bedroom" in said


def test_every_miss_is_reported_at_once():
    structure = _structure(shots=[_shot(characters=["ghost"]), _shot(location="rooftop")])
    with pytest.raises(BadStructure) as refused:
...
def test_a_structure_with_no_shots_says_so():
    with pytest.raises(BadStructure):
        build_prompts(_structure(shots=[]))
    with pytest.raises(BadStructure):
...
@pytest.mark.parametrize(
    "source,expected",
    [
        ("intro-shots.json", "intro-shots.py"),
        ("scene.json", "scene.py"),
