...
that tool's version. **A version belongs to one tool's counter** — a run that reaches into the other
tool says which items did so in its own header, and claims no shared counter of its own.
