...
def run_queue(runner, store, record, plan_store, producers, now, project, log=None):
    """`producers` is the {job type: producer} map the loop dispatches on; `log` is only carried
    through, because where the loop's timing line lands is main.py's choice."""
    state = runner.status()
