...
| Setup cells for **code** — custom nodes, headless ComfyUI — copied **verbatim**, because that machinery is proven. Into `app.ipynb` when the app needs it to run at all, into `features/producers/data/` when a producer needs it (MMAudio's clone + `pip install -e .`) | Running or importing their cells, or reading a file they own. A copy is not a dependency; its cost is that the two notebooks are maintained separately |
| What each pipeline needs before it can work — the model files, and MMAudio's own repo — written into `features/producers/` as our own list | Their download and install cells. Setting a producer up is the app's own job, not a notebook's ([FOUNDATION 9](FOUNDATION.md)); what we inherit is the names and the addresses, not the machinery |
