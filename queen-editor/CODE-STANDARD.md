...
| Setup cells for **code** — custom nodes, headless ComfyUI — copied **verbatim** into `app.ipynb`, because that machinery is proven | Running or importing their cells, or reading a file they own. A copy is not a dependency; its cost is that the two notebooks are maintained separately |
| Which model files each pipeline needs, and where they come from — written into `features/producers/` as our own list | Their download cells. Installing a model is the app's own job, not a notebook's ([FOUNDATION 9](FOUNDATION.md)); what we inherit is the names and the addresses, not the machinery |
